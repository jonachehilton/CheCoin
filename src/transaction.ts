...
const toHexString = (byteArray): string => Array.from(byteArray, (byte: any) => (`0${(byte & 0xFF).toString(16)}`).slice(-2)).join('');

const getTxInAmount = (txIn: TxIn, aUnspentTxOuts: UnspentTxOut[]): number => findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts).amount;

const getPublicKey = (aPrivateKey: string): string => ec.keyFromPrivate(aPrivateKey, 'hex').getPublic().encode('hex');

const findUnspentTxOut = (transactionId: string, index: number, aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut => aUnspentTxOuts.find((uTxO) => uTxO.txOutId === transactionId && uTxO.txOutIndex === index);
...
function validateBlockTransactions(aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[], blockIndex: number): boolean {
...
function hasDuplicates(txIns: TxIn[]): boolean {
...
function processTransactions(aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[], blockIndex: number) {
...
function getCoinbaseTransaction(address: string, blockIndex: number): Transaction {
...
export {
  processTransactions, signTxIn, getTransactionId, isAddressValid, validateTransaction,
